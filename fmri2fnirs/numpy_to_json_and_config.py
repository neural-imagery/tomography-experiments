...
def get_optodes(vol:np.ndarray):
    """
    Parameters
    ----------
    vol : np.ndarray
    ...
    """
    def padz_with0layer(arr3):
        return np.vstack([arr3.T, np.zeros([1,120,120])]).T
    ...
    N=50 # number of optodes to place
    def get_probes(n=N,vertices=vertices):
        ...
        return probes
    def get_normals(emmitors, vertices):
    ...
    detectors = get_probes(80)                                                 # Initiate 80 detectors
    sources = get_probes(10)                                                   # Initiate 10 probes
...
def arr3_to_mcx_input(numpy_fname:str, json_boilerplate:str="colin27.json"):
    ...
    vol_encoded = jd.encode(vol, {'compression':'zlib','base64':1})            # serialize volume
    ...
    json_inp["Optode"] = {
        "Source": sources_list[0],                                             # For the json we just pick one source, just for mcx viz 
